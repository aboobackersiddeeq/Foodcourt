  ...
  if ($('#north-america-chart-dark').length) {
    ...
    var northAmericaChartPlugins = {
      beforeDraw(chart) {
        ...
        ctx.restore();
        const fontSize = 3.125;
        ctx.font = `500 ${fontSize}em chart4($);sans-serif`;
        ctx.textBaseline = 'middle';
        ctx.fillStyle = '#fff';
  ...
  $.ajax(({
    ...
    success: (res) => {
      ...
      res.sales.forEach((el) => {
        count.push(el.count);
        day.push(`${el._id.year}/${el._id.month}/${el._id.day}`);
      });
      // res.salesRep.forEach((element) => {
      //   month.push(element._id.month);
      //   console.log(element._id.month);
      ...
      can.removeChild(can.childNodes[2]);
      const canv = document.createElement('canvas');
      canv.id = 'sales-chart2';
...
function chart3($) {
  $.ajax(({
    ...
    success: (res) => {
      ...
      //   console.log(element._id.month);
      // });
      const cans = document.getElementById('can');
      cans.removeChild(cans.childNodes[1]);
      const canv = document.createElement('canvas');
      canv.id = 'sales-chart3';
      document.getElementById('can').appendChild(canv);

      var SalesChartCanvas = $('#sales-chart3').get(0).getContext('2d');
      var SalesChart = new Chart(SalesChartCanvas, {
        type: 'bar',
        data: {
          labels: year,
...
function chart4($) {
  $.ajax(({
    ...
    success: (res) => {
      ...
      const count = [];
      console.log(res);
      // res.revenue.forEach((el) => {
      //   year.push(el._id.year);
      // });
      res.sales.forEach((element) => {
        month.push(element._id.month);
